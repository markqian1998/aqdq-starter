...
def load_trade_config(path: Union[str, Path], *, spot_override: Optional[float] = None) -> TradeConfig:
    ...
    spot = float(spot_override) if spot_override is not None else (
        float(spot_yaml) if spot_yaml is not None else None
    )
    ...
    elif strike_pct is not None and spot is not None:
        forward_price = float(spot) * float(strike_pct)
    else:
        raise ValueError(
    ...
    elif ko_pct is not None and spot is not None:
        ko_level = float(spot) * float(ko_pct)
    else:
        raise ValueError(
    ...
    # ---- build AQDQSchedule ----
    # Effective date defaults to pricing_date; final_accum_date = pricing_date + tenor_months.
    effective_date = _ensure_date(t.get("effective_date", pricing_date))
    final_accum_date = _ensure_date(
        t.get("final_accum_date", pricing_date + timedelta(days=tenor_months * 30))
    )
    schedule = AQDQSchedule(
